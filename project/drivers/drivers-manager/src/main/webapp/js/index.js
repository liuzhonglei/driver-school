...
//          console.log("titleObjtop=="+titleObj.offset().top);
    ...
    createMenu: function(data,rootUrl){
        ...
        for(var i=0;i<dataLength;i++){
            ...
            var li = $('<li role="'+role+'"></li>');
            if(children && children.length>0){
                children.sort(function(x,y){return x.sort - y.sort});

                li.html('<a class="dropdown-toggle '+role+'" href="#"> '+name+' <span class="fa fa-angle-down pull-right"></span></a>');
                if(children.length > 0){
                    var childrenUl=$('<ul class="nav sub"></ul>');
                    for(var j=0;j<children.length;j++){
                        var childrenItem = children[j];
                        var childrenChildren=childrenItem.children;

                        var grandsonLi=$('<li><a href="'+rootUrl+childrenItem.url+'">'+childrenItem.name+'</a>');
                    ...
                    li.append('<li><a href="'+rootUrl+url+'">'+name+'</a>');
                }
    ...
    showLoading: function () {
        if ($("#loading").length > 0) {
            ...
            var loading = '<div id="loading"><img src="/b_kry/themes/style/img/loading.gif" /></div>';
    ...
    number: function () {
        ...
        $(document).delegate("input[data-type='number']", "keyup change", function (e) {
...
//            alert(pos);
     ...
     */
    digits: function () {
        $(document).delegate("input[data-type='digits']", "keyup change", function (e) {
            var reg = /\D/g;
            bkeruyun.limitInputFormat(this, reg);
        });
    },
...
//        });
    ...
    clearValue: function () {
        ...
        $(document).delegate(".search-box .close", "click", function () {
            ...
            if(window.JPlaceHolder){
                inputObj.parent(".placeholderBox").find(".holder").show();
            }
        });
    },
    /**
...
//            console.log("_endDate ==" + _endDate);
        ...
        if(window.JPlaceHolder){
            that.clearJplaceholder("input.datepicker-start");
            that.clearJplaceholder("input.datepicker-end");
        }
    },
    /**
     ...
     * @expression {string}
     */
    clearJplaceholder:function(expression){
...
//  });
    ...
    //初始化日历控件
    bkeruyun.datepickerInitialize();
    //初始化链接当前状态

    //bkeruyun.currentLink();
    //初始化导航
